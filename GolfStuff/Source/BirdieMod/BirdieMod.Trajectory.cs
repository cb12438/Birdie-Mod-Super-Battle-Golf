...
public partial class BirdieMod
{
    private void UpdateTrails()
    {
        if (!tracersEnabled)
        {
            if (shotPathLine != null && shotPathLine.positionCount != 0)
            {
    ...
    {
        // AutoAimCamera (OnLateUpdate) sets the orbit camera's targetYaw toward currentAimTargetPosition
        // and then writes that yaw back to playerGolfer.transform via ApplyAimDirectionToSwingSubject.
        // But this file runs in OnUpdate — one phase before that write — so playerGolfer.transform.forward
        // is always one frame stale. Use currentAimTargetPosition directly: it is the exact aim point the
        // orbit camera is converging to (with wind compensation already applied), so it matches what
        // AlignWithCameraImmediately() will read at the moment the swing is released.
        Vector3 horizontalDirection;
        if (currentAimTargetPosition != Vector3.zero)
        {
            Vector3 flatToTarget = currentAimTargetPosition - shotOrigin;
            flatToTarget.y = 0f;
            if (flatToTarget.sqrMagnitude > 0.0001f)
            {
                horizontalDirection = flatToTarget.normalized;
            }
            else
            {
                Vector3 pf = playerGolfer != null ? playerGolfer.transform.forward : Vector3.forward;
                horizontalDirection = new Vector3(pf.x, 0f, pf.z);
                if (horizontalDirection.sqrMagnitude < 0.0001f) horizontalDirection = Vector3.forward;
                else horizontalDirection.Normalize();
            }
        }
        else
        {
    ...
    private void BuildPredictedTrajectoryPoints(float shotPower, float swingPitch, List<Vector3> outputPoints)
    {
        ...
        float launchSpeed = Mathf.Max(0.1f, EstimateLaunchSpeedFromPower(shotPower, swingPitch));
        ...
        for (int i = 0; i < predictedPathMaxSteps && elapsed <= predictedPathMaxTime; i++)
        {
            ...
            {
                Vector3 windParallel = Vector3.Project(wind, velocity);
                Vector3 windCross = wind - windParallel;
                Vector3 windEffect = windParallel * windFactor + windCross * crossWindFactor;
                Vector3 relVelocity = velocity - windEffect;
                float relSpeedSq = relVelocity.sqrMagnitude;
                velocity -= relVelocity * (airDragFactor * relSpeedSq * dt);
            }
    ...
    private float EstimateLaunchSpeedFromPower(float power, float swingPitch)
    {
        // Exact game formula: GetSwingHitSpeed returns power * MaxPowerSwingHitSpeed (or
        // MaxPowerPuttHitSpeed for putts), then HitWithGolfSwingInternal multiplies by
        // MatchSetupRules.GetValue(SwingPower).
        InitializeSwingHittableReflection();
        float clampedPower = Mathf.Clamp(power, 0.05f, 2f);
        float currentSrvMul;
        ...
        bool isPutt = Mathf.Abs(swingPitch) < 0.5f;
        float maxSpeed = isPutt ? cachedMaxPowerPuttHitSpeed : cachedMaxPowerSwingHitSpeed;
        return clampedPower * maxSpeed * Mathf.Max(0.01f, currentSrvMul);
    }
    ...
    private void InitializeSwingHittableReflection()
    {
        if (swingHittableReflectionInitialized && ReferenceEquals(cachedGolfBallForHittable, golfBall))
        {
            return;
        }

        swingHittableReflectionInitialized = true;
        cachedGolfBallForHittable = golfBall;
        ...
        try
        {
            ...
            Component hittable = golfBall.GetComponent(hittableType);
            if (hittable == null)
            {
                return;
            }

            object swingSettings = ModReflectionHelper.GetMemberValue(hittable, "SwingSettings");
            if (swingSettings != null)
            {
                float speed = ModReflectionHelper.GetFloatMemberValue(swingSettings, "MaxPowerSwingHitSpeed", 85f);
                if (!float.IsNaN(speed) && !float.IsInfinity(speed) && speed > 0.1f)
                {
                    cachedMaxPowerSwingHitSpeed = speed;
                }

                float puttSpeed = ModReflectionHelper.GetFloatMemberValue(swingSettings, "MaxPowerPuttHitSpeed", 15f);
                if (!float.IsNaN(puttSpeed) && !float.IsInfinity(puttSpeed) && puttSpeed > 0.1f)
                {
                    cachedMaxPowerPuttHitSpeed = puttSpeed;
                }
            }

            object hittableSettings = ModReflectionHelper.GetMemberValue(hittable, "settings");
            if (hittableSettings != null)
            {
                object windSettings = ModReflectionHelper.GetMemberValue(hittableSettings, "Wind");
                if (windSettings != null)
                {
                    float wf = ModReflectionHelper.GetFloatMemberValue(windSettings, "WindFactor", 1f);
                    float cwf = ModReflectionHelper.GetFloatMemberValue(windSettings, "CrossWindFactor", 1f);
    ...
    private void InitializeWindReflection()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < assemblies.Length; i++)
            {
                ...
                cachedWindManagerWindProperty = windManagerType.GetProperty("Wind", BindingFlags.Public | BindingFlags.Static);
    ...
    private Vector3 SimulateLandingPosition(Vector3 shotOrigin, float shotPower, float swingPitch, Vector3 aimTarget, Vector3 wind)
    {
        Vector3 toAim = aimTarget - shotOrigin;
        Vector3 horizontal = new Vector3(toAim.x, 0f, toAim.z);
        Vector3 horizontalDir = horizontal.sqrMagnitude > 0.0001f
        ...
        float pitchRad = swingPitch * Mathf.Deg2Rad;
        Vector3 shotDirection = (horizontalDir * Mathf.Cos(pitchRad) + Vector3.up * Mathf.Sin(pitchRad)).normalized;
        ...
        for (int i = 0; i < predictedPathMaxSteps && elapsed <= predictedPathMaxTime; i++)
        {
            ...
            if (!ascending && position.y <= aimTarget.y)
            {
                float prevY = prevPosition.y;
                float currY = position.y;
                float targetY = aimTarget.y;
                if (!Mathf.Approximately(currY, prevY))
                {
                    float t = Mathf.Clamp01((targetY - prevY) / (currY - prevY));
    ...
    private float EvaluateVerticalAtHorizontalDistanceWithDrag(float launchSpeed, float pitchRad, float targetDistance, float airDragFactor, float deltaTime, float gravityY, float windAlongShot, float windVertical)
    {
        ...
        for (int i = 0; i < 600; i++)
        {
            ...
            float speedSq2d = vx * vx + vy * vy;
            if (speedSq2d > 0.0001f)
            {
                float invSpeedSq = 1f / speedSq2d;
                float dotWV = windAlongShot * vx + windVertical * vy;
                float wpX = dotWV * invSpeedSq * vx;
                float wpY = dotWV * invSpeedSq * vy;
                float wcX = windAlongShot - wpX;
                float wcY = windVertical - wpY;
                float weX = wpX * windFactor + wcX * crossWindFactor;
                float weY = wpY * windFactor + wcY * crossWindFactor;
                float rvX = vx - weX;
                float rvY = vy - weY;
                float relSpeedSq = rvX * rvX + rvY * rvY;
                vx -= rvX * (airDragFactor * relSpeedSq * deltaTime);
                vy -= rvY * (airDragFactor * relSpeedSq * deltaTime);
            }
    ...
    private bool TrySolveRequiredSpeedWithDrag(float horizontalDistance, float heightDifference, float swingPitch, float windAlongShot, float windVertical, out float solvedSpeed)
    {
        solvedSpeed = 0f;
        ...
        float dt = Mathf.Clamp(Time.fixedDeltaTime, 0.005f, 0.04f);
        float rawGravityY = Physics.gravity.y;
        float gravityY = rawGravityY < -0.1f ? rawGravityY : -9.81f;
        float bestSpeed = 0f;
        ...
        {
            float yAtDistance = EvaluateVerticalAtHorizontalDistanceWithDrag(speed, pitchRad, horizontalDistance, airDrag, dt, gravityY, windAlongShot, windVertical);
        ...
        if (hasBracket)
        {
            for (int i = 0; i < 16; i++)
            {
                ...
                float midY = EvaluateVerticalAtHorizontalDistanceWithDrag(midSpeed, pitchRad, horizontalDistance, airDrag, dt, gravityY, windAlongShot, windVertical);
    ...
    // For a putt (SwingPitch == 0), the ball fires horizontally and rolls along the ground.
    // Parabolic solvers return no valid solution at pitch=0 so we use a rolling model instead:
    //   v0 = sqrt(2 * decel * distance),  power = v0 / (maxSpeed * swingMul)
    // puttRollingDecel is an empirical deceleration in m/s² (bounce + grass friction combined).
    private float CalculatePuttPower(float horizontalDistance)
    {
        const float puttRollingDecel = 4f;
        InitializeSwingHittableReflection();
        float currentSrvMul;
        if (!TryGetServerSwingPowerMultiplier(out currentSrvMul))
        {
        ...
        float effectiveMaxSpeed = Mathf.Max(0.1f, cachedMaxPowerPuttHitSpeed * Mathf.Max(0.01f, currentSrvMul));
        float requiredSpeed = Mathf.Sqrt(2f * puttRollingDecel * Mathf.Max(0.01f, horizontalDistance));
        return Mathf.Clamp(requiredSpeed / effectiveMaxSpeed, 0.05f, 2f);
    }

    private float CalculateRequiredPowerForPitch(float horizontalDistance, float heightDifference, float swingPitch, float windAlongShot, float windVertical)
    {
        if (horizontalDistance < 0.01f)
        {
            return 0.05f;
        ...
        // Putt: ball fires horizontally and rolls — parabolic model doesn't apply.
        if (Mathf.Abs(swingPitch) < 0.5f)
        {
            return CalculatePuttPower(horizontalDistance);
        }

        float solvedVelocity;
        if (TrySolveRequiredSpeedWithDrag(horizontalDistance, heightDifference, swingPitch, windAlongShot, windVertical, out solvedVelocity))
        {
    ...
    private void CalculateIdealSwingParameters(bool forceHoleRefresh)
    {
        ...
        try
        {
            ...
            Vector3 wind = GetCurrentWind();
            float windAlongShot = 0f;
            float windVertical = wind.y;

            if (wind.sqrMagnitude > 0.01f && horizontalToTarget.sqrMagnitude > 0.0001f)
            {
                Vector3 shotDir = horizontalToTarget.normalized;
                windAlongShot = Vector3.Dot(wind, shotDir);
                Vector3 crossDir = Vector3.Cross(Vector3.up, shotDir).normalized;
                float windLateral = Vector3.Dot(wind, crossDir);

                if (Mathf.Abs(windLateral) > 0.5f)
                {
                    float initialPower = CalculateRequiredPowerForPitch(horizontalDistance, heightDifference, idealSwingPitch, windAlongShot, windVertical);
                    Vector3 landing = SimulateLandingPosition(shotOrigin, initialPower, idealSwingPitch, currentAimTargetPosition, wind);
                    Vector3 drift = landing - holePosition;
                    float lateralDrift = Vector3.Dot(drift, crossDir);
                    if (Mathf.Abs(lateralDrift) > 0.1f)
                    {
                        windAimOffset = -(crossDir * lateralDrift);
                        currentAimTargetPosition += windAimOffset;
                        ...
                        {
                            shotDir = horizontalToTarget.normalized;
                            windAlongShot = Vector3.Dot(wind, shotDir);
                        }
                    }
                }
            ...
            float physicsPower = CalculateRequiredPowerForPitch(horizontalDistance, heightDifference, idealSwingPitch, windAlongShot, windVertical);
