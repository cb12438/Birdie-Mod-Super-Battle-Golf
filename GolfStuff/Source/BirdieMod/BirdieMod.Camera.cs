...
public partial class BirdieMod
{
    ...
    // Returns true only when the local player is actively in golf-swing aim mode.
    // IsAimingSwing is false whenever any non-golf item (gun, tool, etc.) is equipped,
    // or when the player is in the air, frozen, diving, etc. — so checking this single
    // flag is sufficient to suppress camera control during all non-golf activities.
    private bool IsPlayerAimingSwing()
    {
        ...
        if (!playerGolferProperties.TryGetValue("IsAimingSwing", out aimingProperty) || aimingProperty == null)
        {
            return false;
        }
        ...
        try
        {
            return Convert.ToBoolean(aimingProperty.GetValue(playerGolfer, null));
        }
        catch
        {
            return false;
    ...
    {
        // Only take camera control when the player is in golf-swing aiming mode.
        // This prevents the camera from locking during weapon use, item flourishes, etc.
        bool shouldAim = assistEnabled && (isLeftMousePressed || isRightMousePressed) && IsPlayerAimingSwing();
        ...
        try
        {
            ...
            currentAimTargetPosition = GetAimTargetPosition(aimReferencePosition) + windAimOffset;
