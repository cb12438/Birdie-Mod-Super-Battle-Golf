...
//
// 2. BroadcastWeather(weatherType, weatherAllowed) (host path):
//    • Gets the host's local PlayerInventory netId and ComponentIndex.
//    • Writes byte and bool to a pooled NetworkWriter.
...
//
// 3. Client side — ClientHandleSyncWeather(obj, reader, conn) is the
//    RemoteCallDelegate invoked by Mirror's ClientRpc dispatcher.
//    It reads weatherType and weatherAllowed, then updates ReceivedWeatherType,
//    WeatherAllowedByHost, and ReceivedNewWeather accordingly.

internal static class BirdieWeatherBridge
{
    private const string RpcMethodName =
        "System.Void PlayerInventory::BirdieCmdSyncWeather(System.Byte,System.Boolean)";

    // ── public state ─────────────────────────────────────────────────────────

    internal static byte ReceivedWeatherType;       // 0 = none/clear, 1-8 = weather types
    internal static bool WeatherAllowedByHost = true;
    internal static bool ReceivedNewWeather;        // pulsed true when a broadcast arrives, caller resets
    ...
    private static MethodInfo mirrorWriterExtWriteByte;       // NetworkWriterExtensions.WriteByte
    ...
    private static MethodInfo mirrorReaderExtReadByte;        // NetworkReaderExtensions.ReadByte
    ...
    internal static void EnsureHandlersRegistered()
    {
        ...
        {
            BirdieLog.Warning("[Birdie] Weather bridge init: " + ex.Message);
        }
    }
    ...
    internal static void BroadcastWeather(byte weatherType, bool weatherAllowed)
    {
        ...
        try
        {
            ...
            if (nsType != null)
            {
                ...
                {
                    BirdieLog.Warning("[Birdie] Weather bridge: NetworkServer is not active — cannot broadcast.");
                    return;
                }
            }
        }
        ...
        {
            BirdieLog.Warning("[Birdie] Weather bridge (server check): " + ex.Message);
            return;
        }
        ...
        {
            BirdieLog.Warning("[Birdie] Weather bridge: broadcast not ready (reflection incomplete).");
            return;
        }
        ...
        try
        {
            ...
            if (inventory == null)
            {
                BirdieLog.Warning("[Birdie] Weather bridge: LocalPlayerInventory is null.");
                return;
            }
            ...
            // Serialise byte + bool arguments.
            object writer = mirrorWriterPoolGet.Invoke(null, null);
            mirrorWriterExtWriteByte.Invoke(null, new object[] { writer, weatherType });
            mirrorWriterExtWriteBool.Invoke(null, new object[] { writer, weatherAllowed });
        ...
        {
            BirdieLog.Warning("[Birdie] Weather bridge (broadcast): " + ex.Message);
        }
    }
    ...
    private static void ClientHandleSyncWeather(object nbObj, object readerObj, object connObj)
    {
        try
        {
            if (readerObj == null)
            ...
            if (mirrorReaderExtReadByte == null || mirrorReaderExtReadBool == null)
            {
                BirdieLog.Warning("[Birdie] Weather bridge client handler: reflection incomplete.");
                return;
            }

            byte weatherType = (byte)mirrorReaderExtReadByte.Invoke(null, new object[] { readerObj });
            bool weatherAllowed = (bool)mirrorReaderExtReadBool.Invoke(null, new object[] { readerObj });

            ReceivedWeatherType = weatherType;
            WeatherAllowedByHost = weatherAllowed;
            ReceivedNewWeather = true;

            BirdieLog.Msg("[Birdie] Weather bridge: weatherType=" + weatherType + " weatherAllowed=" + weatherAllowed);
        }
        catch (Exception ex)
        {
            BirdieLog.Warning("[Birdie] Weather bridge client handler: " + ex.Message);
        }
    }
    ...
    private static bool CollectReflectionCaches()
    {
        ...
        mirrorWriterExtWriteByte = mirrorNwExtType.GetMethod(
            "WriteByte",
            BindingFlags.Public | BindingFlags.Static);
        ...
        mirrorReaderExtReadByte = mirrorNrExtType.GetMethod(
            "ReadByte",
            BindingFlags.Public | BindingFlags.Static);
    ...
    private static void RegisterRpcHandler()
    {
        ...
        if (playerInventoryType == null)
        {
            BirdieLog.Warning("[Birdie] Weather bridge: PlayerInventory type not found.");
            return;
        }
        ...
        // The method forwards its args (as object) to ClientHandleSyncWeather.
        DynamicMethod dm = new DynamicMethod(
            "BirdieWeatherRpcHandler",
            typeof(void),
            new Type[] { mirrorNbType, mirrorReaderType, mirrorConnType },
            typeof(BirdieWeatherBridge).Module,
            skipVisibility: true);

        MethodInfo handlerMethod = typeof(BirdieWeatherBridge).GetMethod(
            "ClientHandleSyncWeather",
            BindingFlags.Static | BindingFlags.NonPublic);

        if (handlerMethod == null)
        {
            BirdieLog.Warning("[Birdie] Weather bridge: ClientHandleSyncWeather not found via reflection.");
            return;
        }
        ...
        if (registeredRpcHash == 0)
        {
            BirdieLog.Warning("[Birdie] Weather bridge: RegisterDelegate returned hash 0 — may indicate a collision.");
        }
    }
}
